    ...
    >
      ...
      {arms.map((arm, index) => (
        <fieldset className="grid gap-2 rounded border border-bio-border p-3" key={index}>
          ...
          <label className="text-sm text-bio-text">Mode<select aria-label={`Mode ${index + 1}`} className="mt-1 block rounded border border-bio-border bg-bio-panel-strong px-3 py-2" onChange={(event) => setArms((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, mode: event.target.value as ExperimentArm['mode'], initial_mixer_on: event.target.value === 'manual' ? item.initial_mixer_on : false } : item))} value={arm.mode}><option value="passive">Passive</option><option value="manual">Manual</option></select></label>
