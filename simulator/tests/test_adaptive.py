...
def test_synthetic_trace_reverses_on_worse_objective_and_stays_bounded():
    controller = SyntheticAdaptive(
        AdaptiveConfig(initial_pwm=8, pwm_min=4, pwm_max=16, pwm_step=4)
    )
...
def test_simulator_behavior_is_explicitly_synthetic():
    assert (
        SyntheticAdaptive.__doc__ and "synthetic" in SyntheticAdaptive.__doc__.lower()
    )
